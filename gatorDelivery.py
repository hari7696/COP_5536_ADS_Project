...
class Ordersystem:
    ...
    def func_check_order_deliveries(self):
        ...
        for item in temp_lst:
            ...
            if (
                self.orders_avl.getNode(self.orders_avl.root, item)["eta"]
                < self.current_system_time
            ):
                # deleting the key from the dictionary
                self.f.write(
                    f"Order {item} has been delivered at time {self.orders_avl.getNode(self.orders_avl.root, item)['eta']}\n"
                )
                self.orders_avl.root = self.orders_avl.delete(
                    self.orders_avl.root, item
                )
                self.priority_avl.root = self.priority_avl.delete(
                    self.priority_avl.root,
                    list(temp_lst_dict.keys())[
                        list(temp_lst_dict.values()).index(item)
                    ],
                )
    ...
    def func_update_eta(self, order_id):
        ...
        if len(tmp_orders_priority) >= 1:
            ...
            updated_eta = (
                max(node["creation_time"], self.driver_return_time)
                + node["delivery_time"]
            )

            # updating the node with new eta
            node["eta"] = updated_eta
            self.orders_avl.root = self.orders_avl.update(
                self.orders_avl.root, item, node
            )

            for item in tmp_orders_priority[1:]:

                previous_itm = tmp_orders_priority[tmp_orders_priority.index(item) - 1]
                previous_node = copy.deepcopy(
                    self.orders_avl.getNode(self.orders_avl.root, previous_itm)
                )
                current_node = copy.deepcopy(
                    self.orders_avl.getNode(self.orders_avl.root, item)
                )

                current_node["eta"] = (
                    max(
                        current_node["creation_time"],
                        previous_node["eta"] + previous_node["delivery_time"],
                    )
                ...
                self.orders_avl.root = self.orders_avl.update(
                    self.orders_avl.root, item, current_node
                )
        ...
        if order_id != -100:
            self.f.write(
                "Order {} has been created - ETA: {}\n".format(
                    order_id,
                    self.orders_avl.getNode(self.orders_avl.root, order_id)["eta"],
                )
            )
        ...
        for item in tmp_orders_priority:
            ...
            if (
                self.orders_avl.getNode(self.orders_avl.root, item)["eta"]
                != temp_old_dict[item]["eta"]
            ):
                updated_etas.append(
                    "{}:{}".format(
                        item, self.orders_avl.getNode(self.orders_avl.root, item)["eta"]
                    )
                )
    ...
    def func_create_order(self, order_id, creation_time, order_value, delivery_time):
        ...
        if self.first_order:
            ...
            self.priority_avl.root = self.priority_avl.insert(
                self.priority_avl.root, priority, order_id
            )
            self.orders_avl.root = self.orders_avl.insert(
                self.orders_avl.root, order_id, new_value
            )
            ...
            if (
                self.current_system_time > self.driver_return_time
                and self.priority_avl.getNumberOfNodes() >= 1
            ):

                next_order = list(self.priority_avl.getReverseSortedItems().values())[0]
                node = copy.deepcopy(
                    self.orders_avl.getNode(self.orders_avl.root, next_order)
                )
                node["out_for_delivery"] = True
                self.driver_return_time = node["eta"] + node["delivery_time"]
                self.last_order_eta = node["eta"]
                self.orders_avl.root = self.orders_avl.update(
                    self.orders_avl.root, next_order, node
                )
    ...
    def func_cancel_order(self, order_id, current_system_time):
        ...
        if self.orders_avl.getNode(self.orders_avl.root, order_id) is None:
            self.f.write(
                f"Cannot cancel. Order {order_id} has already been delivered.\n"
            )

        elif self.orders_avl.getNode(self.orders_avl.root, order_id)[
            "out_for_delivery"
        ]:
            self.f.write(f"Order {order_id} is out for delivery\n")

        elif not self.orders_avl.getNode(self.orders_avl.root, order_id)[
            "out_for_delivery"
        ]:
            self.f.write(f"Order {order_id} has been canceled\n")
            self.orders_avl.root = self.orders_avl.delete(
                self.orders_avl.root, order_id
            )

            temp_orders_dict = self.priority_avl.getReverseSortedItems()
            self.priority_avl.root = self.priority_avl.delete(
                self.priority_avl.root,
                list(temp_orders_dict.keys())[
                    list(temp_orders_dict.values()).index(order_id)
                ],
            )
        ...
        if self.orders_avl.getNode(self.orders_avl.root, order_id) is None:
            ...
            for item in orders_priority:
                if item == order_id:
                    ...
                    node["eta"] = (
                        node["eta"] - node["delivery_time"] + new_delivery_time
                    )
                    ...
                    prev_node = self.orders_avl.getNode(
                        self.orders_avl.root, prev_order
                    )
                    current_node = copy.deepcopy(
                        self.orders_avl.getNode(self.orders_avl.root, item)
                    )
    ...
    def func_get_rak_of_order(self, order_id):
        ...
        if order_id in lst_orders:
            self.f.write(
                "Order {} will be delivered after {} orders.\n".format(
                    order_id, lst_orders.index(order_id)
                )
            )
        else:
